...
void ofApp::update() {
  ...
  for(int i = 0; i < nPoints; i++) {
	...
	points[i].x = x;
	points[i].y = y;
	    cloud.addColor(ofColor::fromHsb(ofRandom(255), 255, 255)); // 무작위 색상
...
void ofApp::draw() {
  ...
  if(debugMode) {
	...
	ofSetColor(0, 10);
		//cloud.draw();
		// 기존 코드에서 cloud.draw(); 대신, 다음처럼 변경할 수 있습니다.
	for (int i = 0; i < cloud.getNumVertices(); i++) {
	    glm::vec3 vertex = cloud.getVertex(i);
		ofColor color = cloud.getColor(i);
		ofSetColor(color);
	    ofDrawCircle(vertex.x, vertex.y, 4); // 여기서 2는 원의 반지름, 즉 점의 크기입니다.
	}
